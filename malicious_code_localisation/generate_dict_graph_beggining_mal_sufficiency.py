...
def parseargs():
    ...
    parser.add_argument("-mg", "--malgood", help="'malware' or 'goodware' keyword", type=str, default="malware", required=True)
...
def get_predictions_for_small_segments_sufficiency_beggining(my_dict, sha, index):
    all_steps = [j for j in range(1, 1501, 1)]
    for my_array in get_tf_vect(sha).as_numpy_iterator():
        for step in all_steps:
            my_array_copy = deepcopy(my_array)
            my_array_copy[0][0][step:16384] = [[0]]*int(16384-step)
            predict = model.predict(tf.data.Dataset.from_tensors(my_array_copy))
            predict = get_pred(predict)
            my_dict[step]["0:"+str(step)].append(predict)
...
def main(path_images, model, list_sha, IMG_SIZE, mal_good): 
    ...
    dict_sufficiency["sha"] = []
    dict_sufficiency["index"] = []
    for step in all_steps:
        dict_sufficiency[step] = dict()
        dict_sufficiency[step]["0:"+str(step)] = []
    
    
    list_of_indexes = [i for i in range(len(list_sha))]
    
    for index in list_of_indexes:
        dict_sufficiency = get_predictions_for_small_segments_sufficiency_beggining(dict_sufficiency, list_sha[index], index)
        print("sha with index: %s is processed" % (index))
        save_obj(dict_sufficiency, "dict_sufficiency_beggining_%s.pkl" % mal_good)
        
        
    #plot
    number_of_samples = len(dict_sufficiency["sha"])
    all_steps = [j for j in range(1, 1501, 1)]
    for step in all_steps:
        dict_sufficiency[step]["0:"+str(step)].append((sum(dict_sufficiency[step]["0:"+str(step)])*100)/number_of_samples)
    scores_of_all_steps = [dict_sufficiency[step]["0:"+str(step)][-1] for step in all_steps]
    
    fig = plt.figure(figsize=(20, 5))
    ax = plt.axes()
    ax.plot(all_steps, scores_of_all_steps, c="black");
    ax.set_xlabel('Number of unmasked pixels starting from the beginning of the image', fontsize=22)
    ax.set_ylabel('% of malware detected', fontsize=22)
    ax.tick_params(axis='both', which='major', labelsize=18)
    fig.savefig('sufficiency_malware_first_pixels.pdf', bbox_inches='tight')
